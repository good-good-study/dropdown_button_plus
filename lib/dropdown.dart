...
class _DropdownMenuPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    /// Flutter SDK默认效果: 绘制跟内容一样大小的背景层
    if (!isOutsideShadow) {
      paintSize = rect.size;
    } else {
      /// 新增：内容区域外，绘制阴影层。
      paintSize = Size(rect.size.width, maxSize.height);
    }
    _painter.paint(
      canvas,
      rect.topLeft,
      ImageConfiguration(size: paintSize),
    );
  }
...
class _DropdownMenuItemButtonState<T>
  ...
  Widget build(BuildContext context) {
    ...
    bool select = widget.itemIndex == widget.route.selectedIndex;

    if (select && (widget.isPinnedSelected ?? true)) {
    ...
    if (dropdownMenuItem.enabled) {
      child = Material(
        color: select
            ...
            : widget.unselectItemBackgroundColor,
        child: InkWell(
...
class _DropdownMenuRouteLayout<T> extends SingleChildLayoutDelegate {
  ...
  BoxConstraints getConstraintsForChild(BoxConstraints constraints) {
    ...
    final double width = forceWidth
        ? math.max(constraints.maxWidth, buttonRect.width)
        : math.min(constraints.maxWidth, buttonRect.width);
  ...
  Offset getPositionForChild(Size size, Size childSize) {
    ...
    double dy;
    if (isDropdown) {
      dy = buttonRect.top + buttonRect.height;
    } else {
      dy = menuLimits.top;
    }
    return Offset(left, dy);
  }
    ...
    return LayoutBuilder(
      builder: (BuildContext context, BoxConstraints constraints) {
        return _DropdownRoutePage<T>(
          ...
          isOutsideShadow: isOutsideShadow,
          itemBackgroundColor: itemBackgroundColor,
          unselectItemBackgroundColor: unselectItemBackgroundColor,
        );
      },
    );
  }
